...
void set_user_led(int i) {
  ...
  /*
     ...
     1U); // Led OFF else GPIO_PinWrite(BOARD_USER_LED_GPIO,
     BOARD_USER_LED_GPIO_PIN, 0U); // Led ON
  */
}
...
void clock_init(void) {
  // PHY: clk 50 MHz
  /*
      const clock_enet_pll_config_t config = {.enableClkOutput = true,
     .enableClkOutput500M = true, .loopDivider = 1}; CLOCK_InitEnetPll(&config);
  ...
  /*
     ...
     kGPIO_NoIntmode};
      ...
      GPIO_PinWrite(GPIO1, 4, 1);  // PHY: active
          ...
          static phy_handle_t phyHandle   = {.phyAddr = 0, .mdioHandle =
     &mdioHandle, .ops = &phyksz8081_ops};
